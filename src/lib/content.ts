...
export async function getBlogPosts(options: BlogQueryOptions = {}): Promise<BlogPost[]> {
  try {
    ...
    if (isDatabaseUnavailable(error)) {
      console.warn('Blog posts are unavailable because the database is not configured or reachable.');
      return [];
    }
...
export async function getBlogPost(
  ...
  try {
    ...
    if (isDatabaseUnavailable(error)) {
      console.warn('Blog post is unavailable because the database is not configured or reachable.');
      return null;
    }

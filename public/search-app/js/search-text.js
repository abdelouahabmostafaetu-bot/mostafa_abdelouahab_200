...
function getTextResultKey(item) {
  if (!item) return '';
...
async function searchText(forceOriginal) {
  ...
  try {
    ...
    for (const res of results) {
      ...
      // Merge with dedup
      const seen = new Set(allItems.map(getTextResultKey).filter(Boolean));
      for (const it of items) {
        const key = getTextResultKey(it);
        if (key && !seen.has(key)) {
          seen.add(key);
          allItems.push(it);
        }
    ...
    if (typeof SearchEnhance !== 'undefined' && allItems.length > 0) {
      ...
      if (topIds.length > 0 || inferredTags.length > 0) {
        ...
        if (extraItems.length > 0) {
          const existingKeys = new Set(allItems.map(getTextResultKey).filter(Boolean));
          const newItems = extraItems.filter(i => {
            const key = getTextResultKey(i);
    ...
    if (typeof SearchEnhance !== 'undefined' && allItems.length < 10) {
      ...
      try {
        ...
        if (googleItems.length > 0) {
          const existingKeys = new Set(allItems.map(getTextResultKey).filter(Boolean));
          const newGoogle = googleItems.filter(i => {
            const key = getTextResultKey(i);
            return key && !existingKeys.has(key);
          });
    ...
    if (typeof SearchEnhance !== 'undefined' && SearchEnhance.fetchExternalMathResults && allItems.length < 14) {
      showStatus('Deep web search across math sites...');
      const webStart = performance.now();
      try {
        const webItems = await SearchEnhance.fetchExternalMathResults(q, 8);
        if (thisId !== _textSearchId) return;
        if (webItems.length > 0) {
          const existingKeys = new Set(allItems.map(getTextResultKey).filter(Boolean));
          const newWebItems = webItems.filter(i => {
            const key = getTextResultKey(i);
            return key && !existingKeys.has(key);
          });
          if (newWebItems.length > 0) {
            allItems = allItems.concat(newWebItems);
            stats.sources['web-math'] = newWebItems.length;
          }
        }
      } catch (_) {}
      stats.timing.web = Math.round(performance.now() - webStart);
    }
...
function renderTextResults(items, quota, stats) {
  ...
  showStatus(`Page ${State.currentPage} — ${items.length} results${quotaStr}`);
...
/** Toggle the answers list open/closed */
 if (isCollapsed) {
    setTimeout(() => {
      if (window.MathJax && MathJax.typesetPromise) {
